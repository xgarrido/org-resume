...
A repository for my resume. It uses [[http://orgmode.org][org-mode]], a custom style sheet and a LaTeX
file style. All styles are embedded within this file and a =Makefile= is
provided to produce the =html= and =pdf= files.
...
**** Main stylesheets
:PROPERTIES:
:TANGLE:   pub/stylesheets/styles.css
:END:
***** Google fonts
#+BEGIN_SRC css
...
:PROPERTIES:
:TANGLE: pub/stylesheets/org-pygments.css
:END:
...
#+BEGIN_SRC emacs-lisp
  (defun org-publish-html ()
    (progn
      ...
      (rename-file (expand-file-name "resume.html" (expand-file-name pub-directory))
                   (expand-file-name "index.html" (expand-file-name pub-directory)) t)
      ))
#+END_SRC
